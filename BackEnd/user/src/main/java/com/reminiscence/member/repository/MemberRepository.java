...
//    @Query("SELECT m.email, m.nickname FROM Member m WHERE m.email LIKE CONCAT('%', :key, '%') OR m.nickname LIKE CONCAT('%', :key, '%')")
    @Query(value = "SELECT email, nickname from Member where email like CONCAT('%', :key, '%') or nickname like CONCAT('%', :key, '%')", nativeQuery = true)
